...
| **무료 티어 예산 설계** | 질문당 LLM 호출을 **4회 → 1회**로 압축 (채점 배치화) |
...
| **무료 티어 전제** | Gemini 무료 · Pinecone 무료 · 로컬 CPU 임베딩에서 동작 |
| **측정 후 채택** | 개선은 정량 측정으로 검증하고, 효과 없으면 **되돌린다** |

세 번째 원칙 때문에 시도한 개선의 **75%가 기각**됐습니다. → [6.6 측정해서 기각한 것들](#66-측정해서-기각한-것들)
...
| 영역 | 선택 | 선택 이유 |
|---|---|---|
| **LLM** | Google Gemini 2.5 Flash (Vision 포함) | 무료 티어 제공 + 멀티모달 |
| **Embedding** | BAAI/bge-m3 (로컬, 1024차원) | 한·영 교차언어 검색 지원 |
| **Vector DB** | Pinecone Serverless | 무료 티어, 운영 부담 없음 |
...
무료 티어는 **분당 5회 · 하루 20회**입니다. 질문당 호출 수를 줄이지 않으면 연속 대화가 불가능합니다.
...
### 6.1 무료 티어 — 하루 3~10개 질문

```
Gemini 무료:  하루 20회  AND  분당 5회
질문당 호출:  2~7회
```

연속 대화를 하려면 **결제 활성화가 필요합니다.**

더 근본적인 문제는 질문당 호출을 늘리는 개선 기법(자체 검증 패스, 질의 확장, LLM 심판 평가)이
전부 봉쇄된다는 점입니다. 특히 **LLM 심판 평가가 막히면서 개선을 정량 평가하는 것 자체가
어렵습니다.**

### 6.2 CPU 추론 — 리랭커를 켤 수 없음
...
> ⚠️ 무료 티어에서는 질문 하나에 LLM 을 2~7회 사용합니다.
> 하루 20회 한도이므로 **하루 3~10개 질문**이 실사용 상한입니다.
