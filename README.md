...
> Gemini 무료 티어에는 분당 5회 · 하루 20회 한도가 있습니다. 이 에이전트는 질문 하나에
> LLM 을 **2~7회** 호출합니다 — 검색이 한 번에 통과하면 2~3회지만, 결과가 관련 없다고
> 판정돼 질문을 다시 쓰고 재검색하면 최대 7회까지 늘어납니다(실측 6회).
> 하루 3~10개 질문이 실사용 상한이므로, 연속 대화를 하려면 결제 활성화를 권장합니다.
